...
func TestWrapperBinaryDispatchesIntoCLI(t *testing.T) {
	if os.Getenv("LIMUXD_REMOTE_MAIN_HELPER") == "1" {
		separator := 0
	...
	wrapperPath := filepath.Join(t.TempDir(), "limuxd-remote-current")
	...
	cmd.Env = append(os.Environ(), "LIMUXD_REMOTE_MAIN_HELPER=1")
