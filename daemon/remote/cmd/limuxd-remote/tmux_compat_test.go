...
func TestTmuxWaitForSignalPath(t *testing.T) {
	...
	if !strings.HasPrefix(path, "/tmp/limux-wait-for-") {
...
func TestConfigureAgentEnvironment(t *testing.T) {
	...
	envKeys := []string{
		"LIMUX_CLAUDE_TEAMS_LIMUX_BIN", "PATH", "TMUX", "TMUX_PANE",
		"TERM", "LIMUX_SOCKET_PATH", "LIMUX_SOCKET", "TERM_PROGRAM",
		"LIMUX_WORKSPACE_ID", "LIMUX_SURFACE_ID",
	...
	// Verify socket path
	if os.Getenv("LIMUX_SOCKET_PATH") != "127.0.0.1:54321" {
		t.Errorf("LIMUX_SOCKET_PATH = %q", os.Getenv("LIMUX_SOCKET_PATH"))
	}
	...
	if os.Getenv("LIMUX_WORKSPACE_ID") != "ws-abc" {
		t.Errorf("LIMUX_WORKSPACE_ID = %q", os.Getenv("LIMUX_WORKSPACE_ID"))
	}
	if os.Getenv("LIMUX_SURFACE_ID") != "surf-789" {
		t.Errorf("LIMUX_SURFACE_ID = %q", os.Getenv("LIMUX_SURFACE_ID"))
	}
	// Verify extra env
