...
func shouldRunCLIForInvocation(argv0 string, args []string) bool {
	...
	if !strings.HasPrefix(base, "limuxd-remote") || len(args) == 0 {
		return false
	}
...
func usage(w io.Writer) {
	...
	_, _ = fmt.Fprintln(w, "  limuxd-remote version")
	_, _ = fmt.Fprintln(w, "  limuxd-remote serve --stdio")
	_, _ = fmt.Fprintln(w, "  limuxd-remote cli <command> [args...]")
}
