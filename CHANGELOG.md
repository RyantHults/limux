...
All notable changes to limux are documented here. Entries below `[0.63.1]`
are from the macOS predecessor (cmux, `manaflow-ai/cmux`) and are preserved
for historical context; the Linux rewrite starts from scratch on the
history below.

## [0.1.0-dev] - unreleased

### Initial Linux rewrite

First Rust + GTK4 implementation of limux, replacing the macOS Swift app.
Covers workspaces, panes, splits, browser panels, control socket, D-Bus
scripting, remote SSH sessions with a Go daemon, and a settings UI.
See [`ACTIVITY.md`](ACTIVITY.md) for the phase-by-phase port log and
[`PORT.md`](PORT.md) for the architectural plan.
