...
if (( USE_NIX )); then
    ...
    # re-use the appimagetool download + patchelf dance from the
    # original script (preserved for NixOS users).
    echo ">>> Building AppImage with appimagetool (nix path)..."
    ...
    # Patchelf appimagetool + bundled mksquashfs for NixOS.
    if command -v patchelf >/dev/null 2>&1; then
    ...
    # Nix sets SOURCE_DATE_EPOCH for reproducible builds, but
    # mksquashfs refuses to run when both SOURCE_DATE_EPOCH and
    # timestamp CLI flags are set. Unset it for the appimagetool
    # invocation.
    ...
    # The system path: re-run linuxdeploy's output plugin to seal the
    # patched AppDir into an AppImage. We can't simply call
    # `--output appimage` again because the AppDir has been modified
