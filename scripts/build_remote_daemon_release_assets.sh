...
Builds limuxd-remote release assets for the supported remote platforms and emits:
  limuxd-remote-<goos>-<goarch>[-<suffix>]
  limuxd-remote-checksums[-<suffix>].txt
  limuxd-remote-manifest[-<suffix>].json
...
if ! command -v go >/dev/null 2>&1; then
  echo "error: go is required to build limuxd-remote release assets" >&2
  exit 1
fi
...
rm -f "$OUTPUT_DIR"/limuxd-remote-* "$OUTPUT_DIR"/limuxd-remote-checksums.txt "$OUTPUT_DIR"/limuxd-remote-manifest.json
...
CHECKSUMS_ASSET_NAME="limuxd-remote-checksums${SUFFIX_TAG}.txt"
CHECKSUMS_PATH="${OUTPUT_DIR}/${CHECKSUMS_ASSET_NAME}"
MANIFEST_PATH="${OUTPUT_DIR}/limuxd-remote-manifest${SUFFIX_TAG}.json"
...
: > "$CHECKSUMS_PATH"
ENTRIES_FILE="$(mktemp "${TMPDIR:-/tmp}/limuxd-remote-entries.XXXXXX")"
trap 'rm -f "$ENTRIES_FILE"' EXIT
: > "$ENTRIES_FILE"

for target in "${TARGETS[@]}"; do
  read -r GOOS GOARCH <<<"$target"
  ASSET_NAME="limuxd-remote-${GOOS}-${GOARCH}${SUFFIX_TAG}"
  ...
  BUILD_PATH="${OUTPUT_DIR}/limuxd-remote-${GOOS}-${GOARCH}.build"
  (
    cd "$DAEMON_ROOT"
    GOOS="$GOOS" \
    GOARCH="$GOARCH" \
...
echo "Built limuxd-remote assets in ${OUTPUT_DIR}"
